...
def amenduser():
    ...
    r = requests.get('http://flask_flask-back:5000/user?id=' + id)
    users=r.json()
    for user in users:
       jsonsend=user
...
@app.route('/')
def home():
    r = requests.get('http://flask_flask-back:5000/users')
    return render_template('home.html', users=r.json())

@app.route('/deluser')
def deluser():
    id = request.args.get('id',type=str)
    r = requests.delete('http://flask_flask-back:5000/user?id=' + id)
    return redirect('/')
...
def procnewuser():
   if request.method  == 'POST':
      ...
      x = requests.post('http://flask_flask-back:5000/adduser', json = jsonstr)
      return redirect('/')
   else:
      return render_template('newuser.html') 
...
def procamenduser():
   if request.method  == 'POST':
      ...
      x = requests.post('http://flask_flask-back:5000/user', json = jsonstr)
      return redirect('/')
   else:
      return render_template('newuser.html')
